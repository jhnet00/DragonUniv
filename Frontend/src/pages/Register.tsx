...
const categoryCourseTypes: Record<string, string> = {
  Infrastructure: "전공과목",
  System: "전공과목",
  Network: "전공과목",
  DevOps: "전공과목",
...
export function Register() {
  ...
  const filtered = useMemo(() => courses.filter((course) => {
    ...
    const matchesCourseType = categoryCourseTypes[course.category] === courseType;
    const matchesSeats = !availableOnly || course.seatsLeft > 0;
    return matchesQuery && matchesCategory && matchesCourseType && matchesSeats;
  }), [query, category, courseType, availableOnly]);
  const wishCourses = courses.filter((course) => course.isRegistered);
  const shownRegisteredCourses = registeredCourses.length ? registeredCourses : wishCourses;
  const wishCredits = shownRegisteredCourses.reduce((sum, course) => sum + course.credits, 0);
  ...
  async function loadCourses() {
    ...
    try {
      ...
      setRegisteredCourses(nextRegistrations);
  ...
  return (
    <section className="page-stack">
      ...
      <div className="register-tabs" aria-label="수강신청 메뉴">
        <button className={activeTab === "search" ? "active" : ""} onClick={() => setActiveTab("search")}><Search size={16} /> 개설강좌 조회/신청</button>
        <button className={activeTab === "registered" ? "active" : ""} onClick={() => setActiveTab("registered")}><ClipboardList size={16} /> 나의 수강신청 현황</button>
      </div>
      {activeTab === "search" ? (
        <>
        ...
        <div className="registration-console">
          <div className="console-header green">
            <span><Bookmark size={16} /> 나의 수강신청 현황</span>
            <em>신청학점: <strong>{wishCredits}</strong>학점 | 신청과목수: <strong>{shownRegisteredCourses.length}</strong></em>
          </div>
          <div className="wish-summary">
            {shownRegisteredCourses.map((course, index) => (
              <article key={course.id}>
                ...
                <button onClick={() => cancelCourse(course)} disabled={busyId === course.classId}>
                  {busyId === course.classId ? "처리중" : "철회"}
                </button>
              </article>
            ))}
            {shownRegisteredCourses.length === 0 ? <p className="empty-line">아직 신청한 강의가 없습니다.</p> : null}
          </div>
          <div className="console-notice">
            수강 철회 후에는 잔여좌석과 신청학점이 즉시 다시 계산됩니다.
          </div>
        </div>
      )}
      <div className="registration-console">
        <div className="console-header green">
          ...
          <em>관심신청학점: <strong>{wishCredits}</strong>학점 | 관심과목수: <strong>{shownRegisteredCourses.length}</strong></em>
        </div>
        <div className="wish-summary">
          {shownRegisteredCourses.map((course, index) => (
            <article key={course.id}>
          ...
          {shownRegisteredCourses.length === 0 ? <p className="empty-line">관심과목으로 표시할 신청 강의가 없습니다.</p> : null}
        </div>
        <div className="console-notice">
